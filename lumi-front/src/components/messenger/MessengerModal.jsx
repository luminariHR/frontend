        ...
        <div className="flex m-auto h-[80%]">
          <div className="ml-10 w-[250px] p-5 bg-[#DCDBFB] shadow-2xl rounded-l-lg">
            <div className="flex flex-col border-b border-gray-300 pb-3">
              ...
              <input
                className="bg-gray-100 text-xs h-6 mt-2 placeholder:pl-1"
                placeholder="Search"
              />
            </div>
            <div className="flex h-[500px] flex-col">
              <h2 className="text-lg font-semibold mt-3 border-b border-gray-300">
                🏢 루미나리
              </h2>
              {users.map((user) => (
                <p
          ...
          <div className="w-[700px] p-5 bg-[#f8f8ff] shadow-lg border-r-2 border-gray-200 flex flex-col justify-between">
            <div className="border-b border-gray-300 pb-3">
              <h2 className="text-lg font-semibold flex flex-row">
                {selectedUser ||
                  chatRooms.find((room) => room.id === chatRoomId)?.title ||
                  "채팅방 선택"}
                <LogOut
                  className="ml-[500px] cursor-pointer"
                  onClick={handleLeave}
                />
                <Plus
            ...
            <div className="flex flex-col overflow-y-auto h-[400px]">
              {messages.map((message, index) => (
                ...
                >
                  ...
                  <span
                    className={`ml-1 text-xs font-medium inline-block p-2 ${message.user === loggedInUser.id ? "bg-[#DCDBFB] rounded-l-full rounded-b-full" : "bg-gray-200 rounded-r-full rounded-b-full"}`}
                  >
                    {message.text}
                  </span>
                </div>
            ...
            <div className="mt-4 h-10 flex border-t border-gray-300 pt-2">
              <div className="flex-grow relative">
                ...
                >
                  <Send
                    className="absolute top-1/3 right-4 text-gray-400"
                    size={20}
                  />
                </button>
              </div>
          ...
          <div className="w-[250px] p-5 bg-[#f8f8ff] shadow-lg rounded-r-lg">
            ...
            <div className="flex flex-col h-[500px] overflow-y-auto">
              {chatRooms.map((chatRoom) => (
                ...
                >
                  <button
                    className={`w-full text-left p-2 rounded-lg ${chatRoom.id === chatRoomId ? "bg-[#8583FD] text-white" : "bg-[#f0f0ff] hover:bg-[#e0e0ff]"}`}
                  >
                    <span>{chatRoom.name}</span>
                    <span className="px-4 flex justify-end text-[11px] font-light">
